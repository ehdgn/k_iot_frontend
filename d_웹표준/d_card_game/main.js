...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function initializeGame() {
    ...
    //? 12개의 카드를 for 반복문으로 생성 HTML 요소로 할당
    for (let i = 0; i < 12; i++) {
      cardContainer.innerHTML += `
        <div class="card">
  ...
  // 2) 카드가 뒤집혔는지 여부
  let hasFlipedCard = false; // 첫 번째 카드가 선택되지 않은 것이 기본값
  ...
  function flipCard() {
    ...
    if(!hasFlipedCard) {
      ...
      hasFlipedCard = false; // 다음 쌍의 선택을 위한 기본값 선언
      secondCard = this;
    }
  ...
  // # 9. 버튼 가시성 토글 함수
  function toggleButtonVisibillity(isGameStarted) {
  ...
  startButton.addEventListener('click', () => {
    ...
    toggleButtonVisibillity(true);

    revealCardTemporay(); // 모든 카드 일시적으로 공개
  ...
  completedButton.addEventListener('click', () => {
    ...
    if (allFlipped) {
      ...
      toggleButtonVisibillity(false); // 시작 버튼만 보이도록
    } else {
      alert('완료되지 않았습니다.');
    }
  });
