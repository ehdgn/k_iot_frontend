...
// EX) useSate  : 상태(state) 기능을 여기서 사용한다.
...
//? 특징
//@ 컴포넌트 단위에서의 상태 관리
//@  상태값(state)과 해당 상태를 업데이트하는 함수를 한 쌍으로 변환
//    >> 상태값 변수명(user), set상태값 변수명(setUser) 
// - 상태 변경 시 컴포넌트는 자동으로 재랜더리
// - 상태 업데이트는 비동기적으로 처리
...
function State01() {
  ...
  //@ state: 현재 할당 값 (변수)
  //@ setState: 상태를 업데이트하는 함
  //@ initialValue: 초기값, 초기 상태를 설정 (설정 가능 - undefined 값)
  // 명명규칙 : set + 상태명 (name -> saetName, count -> setCount, post -> setPost)
