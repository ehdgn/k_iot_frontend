...
export interface Reservation {
  ...
  status: string;     // "CONFIRMED" | "CANCLLED"
}
...
export const useReservationStore = create<ReservationState>((set) => ({
  ...
  fetchReservationById: async (truckId, reservationId) => {
    try {
      ...
      // > set 설정 함수는 내부에 콜백 함수를 가짐 (해당 콜백 함수의 매개변수는 상태의 최신값)
      set((state) => {
        // some() 메서드
        ...
        return {
          reservationList: exists
            ? state.reservationList.map(reservation => (reservationId === dto.id ? mapped : reservation))
