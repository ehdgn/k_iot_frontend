...
function selet(name?: string, nickname: string = '개구리') {
  if (name) {
...
selet(); // 개구리 is guest
selet('이승아'); // 개구리 is 이승아 (name)
selet('강사'); // 개구리 is 강사 (name)
...
selet(undefined, '강사'); // 강사 is guest
selet('이도경', '미어캣'); // 미어캣 is 이도경

//? Rest 나머지 매개변수
// : 함수에 전달하는 여러 개의 매개변수를 그룹화화여 배열로 전달하는 방법
