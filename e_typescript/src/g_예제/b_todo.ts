...
function editTodo(todos: TodoItem[], id: number, task: string) {
  const editTasks = todos.map(todo => todo.id === id ? { ...todo, task: task } : todo);
  return editTasks;
}

function clearCompleted(todos: TodoItem[]): TodoItem[] {
  const clearCompletedTodos = todos.filter(todo => todo.completed === false);
...
function toggleTodo(todos: TodoItem[], id: number): TodoItem[] {
  const toggleTodoCompleted = todos.map(todo => todo.id === id ? { ...todo, completed: !todo.completed } : todo);

  return toggleTodoCompleted;
}

function setAllTodosCompletion(todos: TodoItem[], completed: boolean): TodoItem[] {
  const setAllTodosCompletionAtOnce = todos.map(todo => ({ ...todo, completed}));

  return setAllTodosCompletionAtOnce;
}
