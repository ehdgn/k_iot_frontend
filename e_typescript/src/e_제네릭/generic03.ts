...
function UpdateUser(id: number, changes: Partial<User>) {
  ...
  // 1) name만 있는 경우: { 기존의name, 기존의 age, 변경될 name } >> changes의 name값으로 수정
  // 2) age만 있는 경우: { 기존의name, 기존의 age, 변경될 age } >> changes의 age값으로 수정
  // 3) 둘 다 있는 경우: { 기존의name, 기존의 age, 변경될 name, 변경될 age } >> 모든 값이 changes로 수정
  // 4) 둘 다 없는 경우: 수정 X
